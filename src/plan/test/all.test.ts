import fs from "fs";
import { Plan } from "..";
import { ExecSearch, SplitGaps, SummarizeReport } from "../../exec";
import { Source, SubReport } from "../../state";

const userInput = `Act as a senior cryptocurrency analyst. Conduct a fundamental and market analysis for SUI , a newly listed digital asset. **Output must be a cohesive written report** using formal business language. Structure content strictly according to these sections:  
...
async function testPlan() {
  const tasks: string[] = await Plan({
    user_input: userInput,
  }).then((res) => {
    return res.tasks ?? [];
  });

  const all_sub_reports: SubReport[] = [];

  for (const task of tasks) {
    console.log(`Processing task: ${task}`);

    // Split the task into gaps
    const gaps: string[] = await SplitGaps({
      task: task,
      all_sub_reports: all_sub_reports,
    }).then((res) => {
      return res.gaps ?? [];
    });

    console.log(`Found ${gaps.length} gaps for task`);

    // Search for sources for each gap
    let sources: Source[] = [];
    for (const gap of gaps) {
      console.log(`Searching for gap: ${gap}`);
      const search_results = await ExecSearch(gap);
      sources.push(...search_results);
    }

    console.log(`Found ${sources.length} sources for task`);

    // Generate the sub-report
    const sub_report = await SummarizeReport({
      all_sub_reports: all_sub_reports,
      sources: sources,
      task: task,
    }).then((res) => {
    ...
    if (sub_report) {
      all_sub_reports.push(sub_report);
      console.log(`Completed sub-report: ${sub_report.title}`);
        ...
        JSON.stringify(
          {
            tasks,
            all_sub_reports,
            total_sources: all_sub_reports.reduce(
              (acc, report) => acc + report.sources.length,
              0
            ),
          },
          null,
  ...
  console.log(
    `Completed all ${tasks.length} tasks. Generated ${all_sub_reports.length} sub-reports.`
  );
  return all_sub_reports;
}
